...
var db = new sqlite3.Database('./db/userInfo.db', sqlite3.OPEN_READWRITE, (err) => {
...
app.get("/login", (req, res) => {
  ...
  db.get(`SELECT email, password FROM Users WHERE email = "${email}" AND password = "${password}"`, (err, row) => {
    if (err) {
      console.log(err.message)
    } else {
        try {
